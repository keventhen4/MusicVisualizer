...
while(1):
    ...
    if randint(0,5) == 0: #Object spawning
        meshCount += 1
        meshDataDict[f"mesh{meshCount}"] = gl.MeshData.sphere(rows=4,cols=10,radius=randint(1,10))
        meshItemDict[f"mesh{meshCount}"] = gl.GLMeshItem(
            meshdata = meshDataDict[f"mesh{meshCount}"],
        ...
        meshPosDict[f"mesh{meshCount}"] = [RandomUniform(-10,10),RandomUniform(-10,10),RandomUniform(-10,10)]
        meshColorDict[f"mesh{meshCount}"] = [100, 100, 100]
    ...
    for meshNum in range(1,meshCount+1): #Movement and position update for out-of-bounds detection
        meshItemDict[f"mesh{meshNum}"].translate(
            meshOffsetDict[f"mesh{meshNum}"][0],
            meshOffsetDict[f"mesh{meshNum}"][1],
            meshOffsetDict[f"mesh{meshNum}"][2])
    ...
    for meshNum in range(1, meshCount+1): #Detection + deletion from screen
        for axis in meshPosDict[f"mesh{meshNum}"]:
            if axis > 51 or axis < -51:
                ...
                meshColorDict[f"mesh{meshNum}"][0] -= 10 if meshColorDict[f"mesh{meshNum}"][0] > 10 else 0
                meshColorDict[f"mesh{meshNum}"][1] -= 10 if meshColorDict[f"mesh{meshNum}"][1] > 10 else 0
                meshColorDict[f"mesh{meshNum}"][2] -= 10 if meshColorDict[f"mesh{meshNum}"][2] > 10 else 0
                #meshItemDict[f"mesh{meshNum}"].setMeshData(colors=(1,1,1,meshColorDict[f"mesh{meshNum}"][3]))
                
                meshItemDict[f"mesh{meshNum}"].setColor(
                    QtGui.QColor(
                        meshColorDict[f"mesh{meshNum}"][0],
                        meshColorDict[f"mesh{meshNum}"][1],
                        meshColorDict[f"mesh{meshNum}"][2]
                    )
                )
                break
